...
fn commit_docs(engine: &StorageEngine, body: impl FnOnce(&LocalDocumentStore, &mut Transaction)) {
...
fn increment_accumulates_over_multiple_deltas() {
    ...
    let v = match hits {
        ...
        Value::Document(rmpv::Value::Integer(i)) => i.as_f64().expect("integer convertible to f64"),
        Value::Float(f) => *f,
